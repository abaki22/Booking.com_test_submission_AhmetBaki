...
app.get('/', (req, res) => {
    ...
    } else if (req.query["dropoff"] === undefined) {
        res.status(400);
        res.json({
            status: 400,
            error: "Bad Request",
            message: "Required String parameter 'dropoff' is not present",
            path: "/"
        });
    } else if (req.query["max_customers"] === undefined) {
        res.status(400);
        res.json({
            status: 400,
            error: "Bad Request",
            message: "Required String parameter 'max_customers' is not present",
            path: "/"
        });
    } else {
        res.status(200);
        let params = [];
        let pickup_lat = parseFloat(req.query.pickup.split(",")[0]);
        let pickup_long = parseFloat(req.query.pickup.split(",")[1]);
        let dropoff_lat = parseFloat(req.query.dropoff.split(",")[0]);
        let dropoff_long = parseFloat(req.query.dropoff.split(",")[1]);
        let max_customers = parseInt(req.query.max_customers);

        params[2] = pickup_lat;
        params[3] = pickup_long;
        params[4] = dropoff_lat;
        params[5] = dropoff_long;
        params[6] = max_customers;

        query_suppliers(params).then((result) => res.json({
            status: 200,
